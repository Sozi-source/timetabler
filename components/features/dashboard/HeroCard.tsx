...
import { Bell, ArrowRight, BookOpen, CheckCircle2, Clock, CalendarDays, TrendingUp, Layers } from 'lucide-react'
import { cn } from '@/lib/utils'

// ── Design tokens ──────────────────────────────────────────────────────────────
const TEAL_900  = '#042f2e'
const TEAL_800  = '#134e4a'
...
function WeekArc({
  week, total, size = 80,
}: { week: number; total: number; size?: number }) {
  const r    = (size - 10) / 2
  const cx   = size / 2
  const cy   = size / 2
  const circ = 2 * Math.PI * r
  const pct  = total > 0 ? Math.min(week / total, 1) : 0
  const dash = pct * circ
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} aria-hidden>
      ...
      <text x={cx} y={cy + 9} textAnchor="middle" fontSize="8" fontWeight="500" fill="rgba(255,255,255,0.65)" letterSpacing="0.08em">
        OF {total}
      </text>
    </svg>
  )
...
export default function HeroCard({ term, drafts, published, scheduled }: HeroCardProps) {
  ...
  const week      = term?.week_number     ?? 0
  const total     = term?.teaching_weeks  ?? 0
  const remaining = term?.weeks_remaining ?? 0
    ...
    >
      ...
      <div className="relative px-5 pt-5 pb-4 sm:px-6 sm:pt-6 sm:pb-5 lg:px-8 lg:pt-6 lg:pb-5">
        ...
        <div className="flex items-center justify-between mb-4 sm:mb-5">
          ...
          <button
            className="relative flex h-8 w-8 items-center justify-center rounded-xl border border-white/25 bg-white/15 backdrop-blur-sm text-white transition-all hover:bg-white/25 active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50"
        ...
        <div className="flex items-center justify-between gap-6 mb-4 sm:mb-5">
          ...
          <div className="min-w-0 flex-1">
            {term ? (
              <>
                ...
                <h1 className="text-xl sm:text-2xl lg:text-2xl font-bold text-white leading-tight tracking-tight mb-2.5">
                  {term.name}
                </h1>

                {/* Week progress bar — mobile only */}
                <div className="sm:hidden mb-2.5">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs text-white/60 font-medium">Week progress</span>
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  ...
                  {remaining > 0 && (
                    <span className="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-semibold bg-white/20 text-white">
                      ...
                      {remaining} {remaining === 1 ? 'week' : 'weeks'} left
                    </span>
                  )}
                </div>
              </>
          ...
          {/* Right: week arc — hidden on xs, visible sm+ */}
          {term && total > 0 && (
            <div className="hidden sm:block shrink-0">
              <WeekArc week={week} total={total} size={76} />
            </div>
          )}
        </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex flex-1 items-center justify-center gap-2 rounded-xl py-2.5 px-4 text-sm font-bold transition-all active:scale-[.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50"
            style={{
              background: 'white',
              color: TEAL_700,
          ...
          <button
            ...
            className="flex flex-1 items-center justify-center gap-2 rounded-xl py-2.5 px-4 text-sm font-bold border border-white/30 bg-white/15 text-white backdrop-blur-sm transition-all hover:bg-white/25 active:scale-[.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50"
          >
            Assign Units
