...
function extractContent(responseData: unknown): string {
  ...
  const raw = inner?.content ?? d.content ?? ''
...
function extractContext(responseData: unknown): AIContext | null {
  if (!responseData || typeof responseData !== 'object') return null
  const d = responseData as Record<string, unknown>
  const inner = (d.data ?? d) as Record<string, unknown>
  const ctx = inner.context as AIContext | undefined
  return ctx ?? null
}
...
async function executeAction(
  ...
  try {
    ...
    if (action.action === 'REASSIGN_TRAINER') {
      return {
        ok: true,
        message: `Go to Setup → Curriculum, find ${action.payload.unit_code ?? 'the unit'}, and assign a qualified trainer. Then regenerate.`,
      }
    }
...
function parseResponse(raw: string): { text: string; actions: ActionButton[] } {
  ...
  if (actionsMatch) {
    try { actions = JSON.parse(actionsMatch[1].trim()) } catch { /* ignore */ }
  }
  return { text, actions }
}
            ...
            {message.actions.map((action, i) => (
              ...
              >
                ...
                <div>
                  ...
                  <p className="opacity-70 text-[10px] mt-0.5">{action.description}</p>
                </div>
              </button>
            ))}
          </div>
...
export default function TimetableAI() {
  ...
  async function sendGreeting() {
    ...
    try {
      ...
      const msg =
        err instanceof Error && err.message !== 'Empty response from AI.'
          ? err.message
          : "Hi! I'm your timetable assistant. Ask me about conflicts or how to fix scheduling issues."
  ...
  async function sendMessage(userText?: string) {
    ...
    if (!termId) {
      setMessages(prev => [
        ...prev,
        { role: 'assistant', content: '⚠️ No active term selected. Please select a term first.' },
      ])
      return
    }
    setInput('')
  ...
  async function handleAction(action: ActionButton) {
    ...
    if (result.ok) refreshConflictCount()
  }
      ...
      {open && (
        <div className="fixed bottom-24 right-2 sm:right-6 z-50 w-[calc(100vw-1rem)] sm:w-[380px] max-h-[560px] flex flex-col rounded-2xl bg-white shadow-2xl border border-gray-200 overflow-hidden">
              ...
              <p className="text-xs text-white/60">
                {aiCtx
                  ? conflictCount === 0
                    ...
                    : `${conflictCount} conflict${conflictCount > 1 ? 's' : ''} · ${aiCtx.term_name}`
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 min-h-0">
            ...
            {messages.map((msg, i) => (
              <MessageBubble key={i} message={msg} onAction={handleAction} applying={applying} />
            ))}

            {loading && messages.length > 0 && (
              <div className="flex gap-2 mb-3">
                <div className="shrink-0 h-7 w-7 rounded-full bg-[#1e3a5f] flex items-center justify-center">
                  <Sparkles className="h-3.5 w-3.5 text-amber-300" />
                </div>
          ...
          <div className="border-t border-gray-100 px-3 py-3 shrink-0">
            ...
            <p className="text-[10px] text-gray-300 mt-1.5 text-center">
              Powered by Groq · Always verify AI suggestions before applying
            </p>
          </div>
        </div>
      )}
