...
// Each string must be on one line so Tailwind can detect the classes at build time
...
export default function EntryCard({ entry, colourIndex, onClick }: EntryCardProps) {
  ...
  return (
    <button
      ...
      className={cn(
        'w-full text-left rounded-lg border p-1.5 text-xs',
    ...
    >
      {/* Status dot + unit name */}
      <div className="flex items-center gap-1 mb-0.5">
        <span className={cn('h-1.5 w-1.5 rounded-full shrink-0', STATUS_DOT[entry.status] ?? 'bg-gray-400')} />
        <span className="font-semibold truncate leading-tight">{entry.unit_name}</span>
        {entry.is_combined && (
      ...
      <p className="truncate text-[10px] opacity-60 font-mono">{entry.unit_code}</p>
      <p className="truncate text-[11px] opacity-80">{entry.cohort_name}</p>
