...
interface TrainerTimetableData {
  ...
  days: string[]
  periods: PeriodMeta[]
  // FIX: grid cells can be array, single object, or null
  grid: Record<string, Record<string, SessionEntry | SessionEntry[] | null>>
...
/**
 * FIX: Safely resolve a grid cell regardless of whether the backend
 * returns a single object, an array, or null/undefined.
 * Returns an array of SessionEntry (empty array = free slot).
 */
function resolveCell(
  raw: SessionEntry | SessionEntry[] | null | undefined
): SessionEntry[] {
  if (!raw) return []
...
export default function TrainerTimetablePage({
  ...
  // FIX: default to PUBLISHED so the page shows real data, not empty drafts
  ...
  // Build cohort colour map — FIX: use resolveCell so we handle both shapes
  const cohortColorMap: Record<string, string> = {}
  if (data?.grid) {
    let colorIdx = 0
    for (const day of Object.values(data.grid)) {
      for (const rawCell of Object.values(day)) {
        for (const entry of resolveCell(rawCell)) {
          if (entry.cohort && !cohortColorMap[entry.cohort]) {
            cohortColorMap[entry.cohort] =
              COHORT_COLORS[colorIdx % COHORT_COLORS.length]
            colorIdx++
          }
        }
      }
  ...
  // FIX: count filled cells using resolveCell
  const totalSessions = Object.values(grid).reduce(
    (acc, day) =>
      acc +
      Object.values(day).reduce(
        (dacc, cell) => dacc + resolveCell(cell).length,
        0
      ),
    0
  )
  ...
  const loadPercent = data
    ? Math.round(
        (data.periods_this_week / Math.max(data.max_periods_per_week, 1)) * 100
      )
    : 0
      ...
      {!termId ? (
        ...
        <div className="text-center py-16 rounded-2xl border border-red-100 bg-red-50">
          ...
          <p className="text-xs text-red-400 mt-1">
            Check that this trainer has sessions scheduled.
          </p>
        </div>
      ) : (
        <>
                ...
                <p className="text-sm text-gray-500 mt-0.5">
                  ...
                  {data.staff_id && (
                    <span className="text-gray-400">
                      {' '}
                      ·{' '}
                      <span className="font-mono">{data.staff_id}</span>
                    </span>
                  )}
                  {data.employment_type && (
                    <span className="text-gray-400"> · {data.employment_type}</span>
                  )}
                </p>
              </div>
            </div>
            ...
            <div className="mb-5 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
              <div className="flex items-center justify-between mb-2">
                ...
                <p className="text-xs font-semibold text-gray-600 tabular-nums">
                  {data.periods_this_week} / {data.max_periods_per_week} periods (
                  {loadPercent}%)
                </p>
              </div>
          ...
          {Object.keys(cohortColorMap).length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              {Object.entries(cohortColorMap).map(([cohort, colorClass]) => (
                <span
                  key={cohort}
                  className={`text-xs px-2.5 py-1 rounded-full border font-semibold ${colorClass}`}
                >
                  {cohort}
                </span>
              ))}
                ...
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    ...
                    {days.map((day) => (
                      <th
                        key={day}
                        className="text-center p-3 text-[10px] font-bold uppercase tracking-widest text-gray-500"
                      >
                        {DAY_LABELS[day] ?? day}
                      </th>
                    ))}
                ...
                <tbody>
                  {periods.length === 0 ? (
                    ...
                    periods.map((period, pIdx) => (
                      <tr
                        key={period.id}
                        className={pIdx % 2 === 0 ? 'bg-white' : 'bg-gray-50/40'}
                      >
                        ...
                        {days.map((day) => {
                          // FIX: use resolveCell — handles array, single object, or null
                          const entries = resolveCell(grid[day]?.[period.id])

                          return (
                            <td
                              key={day}
                              className="p-2 align-top border-r border-gray-100 last:border-r-0"
                            >
                              {entries.length > 0 ? (
                                <div className="space-y-1">
                                  {entries.map((entry) => (
                                    ...
                                    >
                                      ...
                                      <p className="mt-1 opacity-60 text-[10px] font-semibold">
                                        {entry.cohort}
                                      </p>
                                      {entry.room && (
                                        <p className="mt-0.5 opacity-50 font-mono text-[10px]">
                                          {entry.room}
                                        </p>
                                      )}
                                      {entry.is_combined && (
          ...
          <p className="text-[10px] text-gray-400 mt-4 text-center font-mono">
            {data.term} ·{' '}
            {viewStatus.charAt(0) + viewStatus.slice(1).toLowerCase()}
          </p>
        </>
      )}
    </div>
