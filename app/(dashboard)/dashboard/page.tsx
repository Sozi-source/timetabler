...
// ── Hero card ─────────────────────────────────────────────────────────────────
function HeroCard({ term, drafts, published, scheduled }: {
  term: { name: string; week_number?: number; teaching_weeks?: number; weeks_remaining?: number } | null
  drafts: number; published: number; scheduled: number
}) {
  const router = useRouter()
  return (
    <div className="relative px-5 pt-10 pb-8 overflow-hidden lg:rounded-2xl" style={{ background: GREEN }}>
      {/* Decorative circles */}
      <div className="absolute -top-10 -right-10 w-48 h-48 rounded-full bg-white/10 pointer-events-none" />
      <div className="absolute top-16 -right-6 w-28 h-28 rounded-full bg-white/10 pointer-events-none" />

      <div className="relative flex items-start justify-between mb-5">
        <div>
          <p className="text-[11px] font-semibold tracking-widest uppercase mb-1 text-white/60">
            Timetable Admin
          </p>
          ...
          {term && (
            <p className="text-xs mt-1 text-white/60">
              Week {term.week_number ?? '—'} of {term.teaching_weeks ?? '—'} · {term.weeks_remaining ?? '—'} weeks left
            </p>
          )}
        </div>
        <button
      ...
      {/* Frosted inner card */}
      <div className="relative rounded-2xl px-5 py-4 bg-white/12 border border-white/20">
        <p className="text-[10px] font-semibold uppercase tracking-widest mb-1 text-white/60">
          Units this term
        </p>
        <p className="text-3xl font-bold text-white tracking-tight">
          {scheduled} <span className="text-base font-normal text-white/60">total</span>
        </p>
        <div className="flex items-end justify-between mt-4 pt-4 border-t border-white/15">
          <div>
            <p className="text-[10px] uppercase tracking-wider font-semibold text-white/60">Published</p>
            <p className="text-lg font-bold text-white mt-0.5">{published}</p>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-wider font-semibold text-white/60">Drafts</p>
            <p className="text-lg font-bold text-white mt-0.5">{drafts}</p>
          </div>
          <button
            onClick={() => router.push('/timetable')}
            className="text-xs font-bold px-4 py-2 rounded-xl text-white bg-white/20 hover:bg-white/30 transition-colors"
          >
            View →
          </button>
        </div>
...
// ── Quick action ──────────────────────────────────────────────────────────────
function QuickAction({ icon: Icon, label, bg, color, onClick, loading }: {
...
// ── Stat tile ─────────────────────────────────────────────────────────────────
function StatTile({ icon: Icon, label, value, bg, color }: {
...
// ── Nav row ───────────────────────────────────────────────────────────────────
function NavRow({ icon: Icon, label, sub, bg, color, onClick }: {
...
// ── Action button ─────────────────────────────────────────────────────────────
function ActionBtn({ icon: Icon, label, description, onClick, loading, disabled, variant }: {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-bold leading-tight" style={{ color: s.text }}>{label}</p>
        <p className="text-[11px] mt-0.5 leading-tight" style={{ color: s.text, opacity: 0.65 }}>{description}</p>
      </div>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-3 px-0.5">{title}</p>
      {children}
    </div>
  )
}
...
export default function DashboardPage() {
  ...
  return (
    <div className="dashboard-page">
      ...
      {/*
        ── Layout strategy ──────────────────────────────────────────────────
        Mobile  : full-bleed hero, single-column content below
        Desktop : hero + content side-by-side in a 2-col grid (or hero full-
                  width on medium, with 3-col content on lg+)
        We use a wrapper that constrains width on xl+ and a responsive grid
        below the hero.
      */}

      {/* ── Mobile hero (full bleed) / Desktop hero inside grid ─────────── */}
...
// ── Extracted sub-sections (reduce repetition between mobile/desktop) ─────────

function MobileQuickActions({ busy, hasConflicts, router, onGenerate, onPublish, onClear }: {
  busy: Action; hasConflicts: boolean
  router: ReturnType<typeof useRouter>
  onGenerate: () => void; onPublish: () => void; onClear: () => void
}) {
  return (
    <Section title="Quick actions">
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4">
        <div className="grid grid-cols-4 gap-2">
          <QuickAction icon={Play}          label="Generate"  bg="#e8f4ee" color={GREEN}    onClick={onGenerate} loading={busy === 'generate'} />
          <QuickAction icon={CheckCircle}   label="Publish"   bg="#eff6ff" color="#1d4ed8"  onClick={onPublish}  loading={busy === 'publish'}  />
          <QuickAction icon={AlertTriangle} label="Conflicts" bg="#fffbeb" color="#d97706"  onClick={() => router.push('/conflicts')} />
          <QuickAction icon={Trash2}        label="Clear"     bg="#fef2f2" color="#b91c1c"  onClick={onClear}    loading={busy === 'clear'}    />
        </div>
      </div>
    </Section>
  )
...
function DesktopActionCard({ busy, hasConflicts, pendingConflicts, draftUnits, canPublish, router, onGenerate, onPublish, onClear }: {
  busy: Action; hasConflicts: boolean; pendingConflicts: number
  draftUnits: number; canPublish: boolean
  router: ReturnType<typeof useRouter>
  onGenerate: () => void; onPublish: () => void; onClear: () => void
}) {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-3">
      ...
      <ActionBtn
        icon={Play} label="Generate Timetable"
        description="Auto-schedule all units for this term"
        onClick={onGenerate} loading={busy === 'generate'} disabled={!!busy} variant="primary"
      />
      <ActionBtn
        icon={CheckCircle} label="Publish Drafts"
        description={
          canPublish
            ? `${draftUnits} draft${draftUnits > 1 ? 's' : ''} ready to publish`
            : hasConflicts ? 'Resolve conflicts before publishing' : 'No drafts to publish'
        }
        onClick={onPublish} loading={busy === 'publish'} disabled={!!busy || !canPublish} variant="success"
      />
      {hasConflicts && (
        <ActionBtn
          icon={AlertTriangle} label="Resolve Conflicts"
          description={`${pendingConflicts} scheduling conflict${pendingConflicts > 1 ? 's' : ''} pending`}
          onClick={() => router.push('/conflicts')} variant="warning"
        />
      )}
      {draftUnits > 0 && (
        <ActionBtn
          icon={Trash2} label="Clear Drafts"
          description="Permanently delete all draft entries"
          onClick={onClear} loading={busy === 'clear'} disabled={!!busy} variant="danger"
        />
      )}
    </div>
  )
...
function StatusSection({ publishedUnits, draftUnits, scheduledUnits, hasConflicts, pendingConflicts, router }: {
  publishedUnits: number; draftUnits: number; scheduledUnits: number
  hasConflicts: boolean; pendingConflicts: number
  router: ReturnType<typeof useRouter>
}) {
  return (
  ...
  return (
    <Section title="Quick access">
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-1 divide-y divide-gray-50">
        <NavRow icon={LayoutGrid}    label="Master Timetable" sub="Full schedule view"   bg="#e8f4ee" color={GREEN}   onClick={() => router.push('/timetable')}          />
        <NavRow icon={GraduationCap} label="By Cohort"        sub="Per-cohort view"      bg="#eff6ff" color="#1d4ed8" onClick={() => router.push('/timetable/cohorts')}  />
        <NavRow icon={Users}         label="By Trainer"       sub="Per-trainer schedule" bg="#f5f3ff" color="#7c3aed" onClick={() => router.push('/timetable/trainers')} />
        <NavRow icon={ShieldCheck}   label="Constraints"      sub="Scheduling rules"     bg="#fef2f2" color="#b91c1c" onClick={() => router.push('/constraints')}        />
      </div>
    </Section>
  )
}
